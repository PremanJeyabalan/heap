...
namespace heep::helpers {
    ...
    static constexpr uint32_t pack_mask = 0xFFFFFFF8;

    static constexpr size_t get_aligned_block(size_t bytes) {
        return get_aligned_16_bytes(alloc_header_size + get_aligned_16_bytes(bytes) + footer_size);
    }
    ...
    static constexpr void* get_block_footer_address_from_start(void* ptr, uint32_t size) {
        return static_cast<void*>(
                static_cast<char*>(get_block_end_address_from_start(ptr, size)) - footer_size);
    }
    ...
    static constexpr void* get_block_start_address_from_footer(void* ptr, uint32_t size) {
        return static_cast<void*>(static_cast<char*>(ptr) + footer_size - size);
    }
    static constexpr void* get_block_prev_footer_address(void* ptr) {
        return static_cast<void*>(static_cast<char*>(ptr) - footer_size);
    }
    static constexpr void* get_block_prev_start_address(void* ptr, uint32_t size) {
        return get_block_start_address_from_footer(get_block_prev_footer_address(ptr), size);
    }
}
